"""
ENSO Phase vs Weather Scatter Chart Generator  — All Cities
============================================================
Reads every city tab from a single Excel workbook and writes one
standalone interactive HTML file per city.

Usage
-----
    python3 enso_charts_all_cities.py

Output
------
    ./output/ENSO_Charts_<TAB>.html   (one per city)

To add / remove cities, edit CITY_TABS below.
City display names are read from cell A1 of each tab; the CITY_NAME_FALLBACKS
dict handles the two tabs whose A1 cell is blank (NYC, ABQ).
"""
...
# All city sheet tabs to process
CITY_TABS = [
    "NYC", "BOS", "DCA", "PHL", "ALB", "PAFA", "ABQ", "COS",
    "PWM", "SLC", "FLG", "RNO", "MSP", "ORD", "RIC", "CLE",
    "OMA", "IND", "SEA",
]

# Fallback display names for tabs whose A1 cell is blank
CITY_NAME_FALLBACKS = {
    "NYC": "Central Park, NY",
    "ABQ": "Albuquerque, NM",
}
...
# ── Data loading ─────────────────────────────────────────────────────────────
def get_city_name(excel_file, tab):
    """Read display name from cell A1; fall back to CITY_NAME_FALLBACKS.

    Uses nrows=3 because openpyxl skips fully-blank leading rows, which causes
    some tabs (e.g. NYC, ABQ) to return an empty DataFrame with nrows=1.
    """
    raw = pd.read_excel(excel_file, sheet_name=tab, header=None, nrows=3)
    if raw.empty or pd.isna(raw.iloc[0, 0]) or str(raw.iloc[0, 0]).strip() == "":
        return CITY_NAME_FALLBACKS.get(tab, tab)
    return str(raw.iloc[0, 0]).strip()


def load_data(excel_file, tab):
    """Load and clean city data.

    Handles NOAA sentinel values that appear in weather columns:
      'T'  — trace amount (< 0.05 in snow / < 0.005 in rain) → treated as 0.0
      'M'  — missing observation → treated as NaN (row is dropped)
    """
    df = pd.read_excel(excel_file, sheet_name=tab, header=2, dtype=str)
    df.columns = ["Season", "Temp", "Snowfall", "ENSO_Code"]

    # Replace NOAA sentinels before numeric conversion
    df["Temp"]     = df["Temp"].replace("T", "0").replace("M", np.nan)
    df["Snowfall"] = df["Snowfall"].replace("T", "0").replace("M", np.nan)

    # Coerce to numeric (any remaining non-numeric becomes NaN)
    df["Temp"]      = pd.to_numeric(df["Temp"],      errors="coerce")
    df["Snowfall"]  = pd.to_numeric(df["Snowfall"],  errors="coerce")
    df["ENSO_Code"] = pd.to_numeric(df["ENSO_Code"], errors="coerce")
...
def add_jitter(df, seed=JITTER_SEED):
...
def build_app_data(df, city_name):
...
function buildChart(cid) {
  ...
  document.getElementById(`plt-${cid}`).on('plotly_click', ev => {
    ...
    if (ST[cid].decade) {
      const clickedDecade = ev.points[0].customdata[2];
...
def main():
    ...
    for tab in CITY_TABS:
        try:
            city_name = get_city_name(EXCEL_FILE, tab)
            df        = load_data(EXCEL_FILE, tab)
            df        = add_jitter(df)
            app_data  = build_app_data(df, city_name)
            ...
            ok.append((tab, city_name, len(df)))
            print(f"  ✓  {tab:6s}  {city_name}  ({len(df)} seasons)")

        except Exception as e:
            skipped.append((tab, str(e)))
            print(f"  ✗  {tab:6s}  ERROR: {e}")

    print(f"\nDone — {len(ok)} files written to '{OUTPUT_DIR}/'")
    if skipped:
